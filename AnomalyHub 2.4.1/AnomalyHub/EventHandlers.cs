...
namespace AnomalyHub
{
    public class EventHandlers
    {
        ...
        public void NewHealths(SpawningEventArgs ev)
        {
            ...
            {
                ev.Player.Health = Plugin.Instance.Config.SCP096newHP;
            }
            if (ev.Player.Role == RoleTypeId.Scp106)
            {
                ev.Player.Health = Plugin.Instance.Config.SCP106newHP;
            }
            if (ev.Player.Role == RoleTypeId.Scp173)
            {
                ev.Player.Health = Plugin.Instance.Config.SCP173newHP;
            }
            if (ev.Player.Role == RoleTypeId.Scp939)
            {
                ev.Player.Health = Plugin.Instance.Config.SCP939newHP;
            }
            if (ev.Player.Role == RoleTypeId.Scp049)
            {
                ev.Player.Health = Plugin.Instance.Config.SCP049newHP;
            }
        }
